...
/*   Updated: 2020/12/18 11:45:27 by coscialp         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int		init_fork(int i)
{
	static t_fork	*left = NULL;
	static t_fork	*top = NULL;

	if (!(state()->fork = malloc(sizeof(t_fork))))
		return (1);
	state()->fork->left = left;
	if (left)
		state()->fork->left->right = state()->fork;
	state()->fork->available = 0;
	state()->fork->id = i + 1;
	pthread_mutex_init(&state()->fork->mutex, NULL);
	left = state()->fork;
	if (i == 0)
		top = state()->fork;
	if (i == state()->rules.number_of_philo - 1)
		state()->fork->right = top;
	return (0);
}
...
int		init_philosoph(int i)
{
	...
	state()->philosoph->r_fork = state()->fork;
	state()->philosoph->eating = 0;
	pthread_mutex_init(&state()->philosoph->mutex_check, NULL);
	pthread_mutex_init(&state()->philosoph->mutex_eat, NULL);
...
int		init_rules(int ac, char **av)
{
	...
	pthread_mutex_init(&state()->mutex_write, NULL);
	pthread_mutex_init(&state()->mutex_state, NULL);
	pthread_mutex_init(&state()->mutex_fork, NULL);
	return (0);
}
...
int		init_state(int ac, char **av)
{
	...
	{
		if (init_fork(i) || init_philosoph(i))
			return (ft_error("Allocation failed\n"));
	}
	i = -1;
	while (++i < state()->rules.number_of_philo)
	{
		state()->philosoph->l_fork = state()->fork->right;
		state()->philosoph = state()->philosoph->right;
		state()->fork = state()->fork->right;
	}
	state()->philosoph = state()->philosoph->right;
	return (0);
}
